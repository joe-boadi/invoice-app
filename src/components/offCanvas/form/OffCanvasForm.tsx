...
// import { updateFormData } from "../../../redux/form/formDataSlice";
...
const OffCanvasForm = ({
      ...
      Object.entries(formData).forEach(([key, value]) => {
        ...
        if (typeof value === "string") {
          ...
          Object.entries(value).forEach(([key2, value2]) => {
            if (key === "items") {
              const validatedItem = Object.entries(value2).reduce(
                (acc: any, [key3, value3]) => {
                  if (key3 === "id") {
                    return {
                      ...acc,
  ...
  const submitData = (status: string) => {
    ...
    //Toaster or Snackbar notification
    showNotification("Invoice Successfully Added", "success");
    close();
  ...
  const handleSubmit = useCallback(() => {
    if (formIsSaved) {
      ...
      submitData("draft");
      showNotification("Draft Successfully Added", "success");
    }
  }, [formIsSaved, formData, formError, submitData]);

  useEffect(() => {
    if (formIsSaved) {
      validateFormErrors();
    }
  }, [formData, formIsSaved]);

  useEffect(() => {
    if (updateForm) {
      updateForm(formData);
      // showNotification("Inovice Successfully Updated", "info");
    }
  }, [formData, showNotification, updateForm]);
